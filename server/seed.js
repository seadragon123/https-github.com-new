...
for (const p of incenseProductData) {
...
// ─── 支出示例 ───
const expenseData = [
  ['2026-06-27', '日常耗材', 150, '买纸巾、洗浴用品'],
  ['2026-06-27', '餐饮成本', 320, '蔬菜肉类采购'],
  ['2026-06-27', '维修杂费', 50, '灯泡更换'],
  ['2026-06-26', '日常耗材', 200, '床单换新'],
]
...
console.log(`  - ${roomData.length} 间客房`)
console.log(`  - ${guestData.length} 位客人`)
console.log(`  - ${bookingData.length} 条预订`)
console.log(`  - ${depositData.length} 条押金记录`)
console.log(`  - ${menuData.length} 道菜品`)
console.log(`  - ${incenseProductData.length} 个请香商品`)
console.log(`  - ${incSales.length} 条请香销售`)
console.log(`  - ${catOrders.length} 条餐饮订单`)
console.log(`  - ${expenseData.length} 条支出记录`)
console.log(`  - ${todoData.length} 条待办`)
console.log(`  - 2 条维修 | 4 条清洁`)
