...
const uploadsDir = process.env.UPLOADS_DIR || join(__dirname, '..', 'uploads', 'receipts')
...
router.get('/', (req, res) => {
  ...
  // 当日支出
  let dailySql = 'SELECT * FROM expenses WHERE report_date = ?'
  const dailyParams = [d]
  ...
  for (const cat of EXPENSE_CATEGORIES) {
    const mRows = queryAll(
      'SELECT COALESCE(SUM(amount), 0) as total FROM expenses WHERE category = ? AND report_date LIKE ?',
      [cat, m + '%']
    )
...
router.post('/', upload.single('receipt_image'), (req, res) => {
  ...
  const id = insertAndGetId(
    'INSERT INTO expenses (report_date, category, amount, note, receipt_image) VALUES (?, ?, ?, ?, ?)',
    [report_date, category, amount, note || '', receiptImage]
  )
...
router.put('/:id', upload.single('receipt_image'), (req, res) => {
  ...
  runSql(
    'UPDATE expenses SET category=?, amount=?, note=?, receipt_image=?, updated_at=datetime(\'now\',\'localtime\') WHERE id=?',
    [category ?? exp.category, amount ?? exp.amount, note ?? exp.note, receiptImage, req.params.id]
  )
