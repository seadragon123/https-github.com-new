    ...
    <div class="page-body">
      ...
      <div class="card">
        ...
        <div class="card-footer flex-between">
          ...
          <span class="stat-num">¥{{ formatMoney(roomTotal) }}</span>
        </div>
      </div>
      ...
      <div class="card">
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="(r, i) in incenseItems" :key="r._key" class="clickable-row" @click="showDetail(r, 'incense')">
                ...
                <td class="text-right">¥{{ formatMoney(r.net_amount || r.amount) }}</td>
                <td class="text-right" :class="(r.commission_amount||0)>0?'text-warning':''">{{ (r.commission_amount||0)>0 ? '¥'+formatMoney(r.commission_amount) : '—' }}</td>
                <td>{{ r.payment_method || '—' }}</td>
              </tr>
        ...
        <div class="card-footer flex-between">
          ...
          <span class="stat-num">¥{{ formatMoney(incenseTotal) }}</span>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-footer flex-between">
          ...
          <span class="stat-num">¥{{ formatMoney(cateringTotal) }}</span>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-footer flex-between">
          ...
          <span class="stat-num text-danger">¥{{ formatMoney(expenseTotal) }}</span>
        </div>
      </div>
          ...
          <table class="data-table summary-table">
            ...
            <tbody>
              <tr class="summary-row">
                ...
                <td class="stat-num">¥{{ formatMoney(summary.daily.roomRevenue) }}</td>
                <td class="text-sm text-muted">= 已完成退房订单</td>
              </tr>
              <tr class="summary-row">
                <td>{{ periodPrefix }}餐饮收入</td>
                <td class="stat-num">¥{{ formatMoney(summary.daily.cateringRevenue) }}</td>
                <td class="text-sm text-muted">= 已结账餐饮订单</td>
              </tr>
              <tr class="summary-row">
                <td>{{ periodPrefix }}请香收入</td>
                <td class="stat-num">¥{{ formatMoney(summary.daily.incenseRevenue) }}</td>
                <td class="text-sm text-muted">= 请香销售（扣除返佣）</td>
              </tr>
              <tr class="summary-row total-row">
                <td><strong>{{ periodPrefix }}总收入</strong></td>
                <td><strong>¥{{ formatMoney(summary.daily.totalRevenue) }}</strong></td>
                <td class="text-sm text-muted">= 客房 + 餐饮 + 请香</td>
              </tr>
              <tr class="summary-row">
                <td>{{ periodPrefix }}总支出</td>
                <td class="stat-num text-danger">¥{{ formatMoney(summary.daily.totalExpense) }}</td>
                <td class="text-sm text-muted">= 支出小计</td>
              </tr>
              <tr class="summary-row net-row">
                <td><strong>{{ periodPrefix }}净现金流</strong></td>
                <td><strong :class="netClass">¥{{ formatMoney(summary.daily.netCashflow) }}</strong></td>
              ...
              <tr class="summary-row">
                <td>本月累计收入</td>
                <td class="stat-num">¥{{ formatMoney(summary.monthly.totalRevenue) }}</td>
                <td></td>
              </tr>
              <tr class="summary-row">
                <td>本月累计支出</td>
                <td class="stat-num text-danger">¥{{ formatMoney(summary.monthly.totalExpense) }}</td>
                <td></td>
              </tr>
              <tr class="summary-row net-row">
                <td><strong>本月累计结余</strong></td>
                <td><strong :class="monthNetClass">¥{{ formatMoney(summary.monthly.netBalance) }}</strong></td>
...
function unitPrice(r) {
  ...
  return qty > 0 ? formatMoney(total / qty) : '0.00'
}
...
const detailFields = computed(() => {
  ...
  if (t === 'room') {
    return {
      ...
      '单价': r.price ? '¥' + formatMoney(r.price) : '—',
    ...
    return {
      '商品名称': r.product_name || '—',
      '数量': r.quantity || 1,
      '销售金额': '¥' + formatMoney(r.amount),
      '返佣比例': r.has_commission ? (r.commission_rate || 0) + '%' : '无',
      '返佣金额': (r.commission_amount || 0) > 0 ? '¥' + formatMoney(r.commission_amount) : '—',
      '实收金额': '¥' + formatMoney(r.net_amount || r.amount),
      '支付方式': r.payment_method || '—',
...
async function autoGenerate() {
  ...
  try {
    ...
    updateRoomTotal()
    refreshMonthlySummary()
    showToast('✅ ' + r.message + '\n共' + r.items.length + '条，合计¥' + formatMoney(r.total))
  } catch (err) {
...
function printReport() {
  ...
  if (roomItems.value.length > 0) {
    ...
    for (const r of roomItems.value) {
      html += `<tr><td>${r.room_no}</td><td>${r.channel_type || '—'}</td><td>${parseGuestName(r.channel_source)}</td><td>¥${formatMoney(r.amount)}</td><td>${r.payment_method || '—'}</td></tr>`
    }
    html += `</table><div class="total">客房收入合计: ¥${formatMoney(roomTotal.value)}</div>`
  }
  // 餐饮收入
  if (cateringItems.value.length > 0) {
    ...
    for (const r of cateringItems.value) {
      html += `<tr><td>${r.guest_name}</td><td>${r.order_type}</td><td>${formatDishNames(r.items)}</td><td>¥${formatMoney(r.total)}</td><td>${r.payment_method || '—'}</td></tr>`
    }
    html += `</table><div class="total">餐饮收入合计: ¥${formatMoney(cateringTotal.value)}</div>`
  }
  // 请香收入
  if (incenseItems.value.length > 0) {
    html += `<h3>三、请香收入</h3><table><tr><th>商品名称</th><th>单价</th><th>数量</th><th>金额</th><th>返佣金额</th><th>支付方式</th></tr>`
    for (const r of incenseItems.value) {
      html += `<tr><td>${r.product_name || '—'}</td><td>¥${unitPrice(r)}</td><td>${r.quantity || 1}</td><td>¥${formatMoney(r.net_amount || r.amount)}</td><td>${(r.commission_amount||0) > 0 ? '¥'+formatMoney(r.commission_amount) : '—'}</td><td>${r.payment_method || '—'}</td></tr>`
    }
    html += `</table><div class="total">请香收入合计: ¥${formatMoney(incenseTotal.value)}</div>`
  }
  // 支出明细
  if (allExpenses.value.length > 0) {
    html += `<h3>四、支出明细</h3><table><tr><th>类别</th><th>金额</th><th>支出日期</th><th>报销人</th><th>备注</th></tr>`
    for (const e of allExpenses.value) {
      if (e.amount > 0) html += `<tr><td>${e.category}</td><td>¥${formatMoney(e.amount)}</td><td>${e.expense_date || '—'}</td><td>${e.reimbursement_person || '—'}</td><td>${e.note || '—'}</td></tr>`
    }
    html += `</table><div class="total">支出合计: ¥${formatMoney(expenseTotal.value)}</div>`
  }
  html += `<h3>经营结果汇总</h3>
  <table>
    <tr><th>项目</th><th>金额(元)</th></tr>
    <tr><td>${prefix}总收入</td><td>¥${formatMoney(summary.value.daily.totalRevenue)}</td></tr>
    <tr><td>${prefix}总支出</td><td>¥${formatMoney(summary.value.daily.totalExpense)}</td></tr>
    <tr><td><strong>${prefix}净现金流</strong></td><td><strong>¥${formatMoney(summary.value.daily.netCashflow)}</strong></td></tr>`
  if (isSingleDay.value) {
    html += `<tr><td>本月累计收入</td><td>¥${formatMoney(summary.value.monthly.totalRevenue)}</td></tr>
    <tr><td>本月累计支出</td><td>¥${formatMoney(summary.value.monthly.totalExpense)}</td></tr>
    <tr><td><strong>本月累计结余</strong></td><td><strong>¥${formatMoney(summary.value.monthly.netBalance)}</strong></td></tr>`
  }
  html += `</table>
...
function exportCSV() {
  ...
  if (roomItems.value.length > 0) {
    csv += `客房收入\n房号,渠道,客户姓名,金额,支付方式\n`
    for (const r of roomItems.value) {
      csv += `${r.room_no},${r.channel_type || '—'},${parseGuestName(r.channel_source)},${formatMoney(r.amount)},${r.payment_method || '—'}\n`
    }
    csv += `,客房收入合计,,¥${formatMoney(roomTotal.value)}\n\n`
  }
  if (cateringItems.value.length > 0) {
    csv += `餐饮收入\n客人姓名,类型,菜品名称,金额,支付方式\n`
    for (const r of cateringItems.value) {
      csv += `${r.guest_name},${r.order_type},"${formatDishNames(r.items)}",${formatMoney(r.total)},${r.payment_method || '—'}\n`
    }
    csv += `,餐饮收入合计,,¥${formatMoney(cateringTotal.value)}\n\n`
  }
  if (incenseItems.value.length > 0) {
    csv += `请香收入\n商品名称,单价,数量,金额,返佣金额,支付方式\n`
    for (const r of incenseItems.value) {
      csv += `${r.product_name || '—'},${unitPrice(r)},${r.quantity || 1},${formatMoney(r.net_amount || r.amount)},${(r.commission_amount||0)>0 ? formatMoney(r.commission_amount) : '—'},${r.payment_method || '—'}\n`
    }
    csv += `,请香收入合计,,,,¥${formatMoney(incenseTotal.value)}\n\n`
  }
  if (allExpenses.value.length > 0) {
    csv += `支出\n类别,金额,备注\n`
    for (const e of allExpenses.value) {
      if (e.amount > 0) csv += `${e.category},${formatMoney(e.amount)},"${e.note || ''}"\n`
    }
    csv += `,支出合计,¥${formatMoney(expenseTotal.value)}\n\n`
  }
  csv += `经营汇总\n项目,金额\n`
  csv += `${prefix}总收入,¥${formatMoney(summary.value.daily.totalRevenue)}\n`
  csv += `${prefix}总支出,¥${formatMoney(summary.value.daily.totalExpense)}\n`
  csv += `${prefix}净现金流,¥${formatMoney(summary.value.daily.netCashflow)}\n`
  if (isSingleDay.value) {
    csv += `本月累计收入,¥${formatMoney(summary.value.monthly.totalRevenue)}\n`
    csv += `本月累计支出,¥${formatMoney(summary.value.monthly.totalExpense)}\n`
    csv += `本月累计结余,¥${formatMoney(summary.value.monthly.netBalance)}\n`
  }
