<template>
  <div class="page">
    ...
    <div class="page-body">
      <!-- 支出分布（联动日期筛选） -->
      <div class="card">
        <div class="card-header">📊 {{ distTitle }}</div>
        <div class="card-body">
          <template v-for="(total, cat) in distData.cats" :key="cat">
          ...
          <div class="dist-total" v-if="distData.total > 0">
            <span>合计 {{ distData.count }} 项</span>
            <span class="dist-total-amount">¥{{ distData.total.toFixed(2) }}</span>
          </div>
          <van-empty v-if="distData.total === 0" description="暂无支出数据" />
        </div>
      </div>
...
const distTitle = computed(() => {
  const s = expDateStart.value, e = expDateEnd.value
  if (!s && !e) return '支出分布'
  if (s === e) return `${s.slice(5)} 支出分布`
  return `${s.slice(5)} - ${e.slice(5)} 支出分布`
})
...
const distData = computed(() => {
  ...
  const count = Object.values(cats).filter(v => v > 0).length
  return { cats, total, count }
})

function pct(total) {
  return distData.value.total > 0 ? Number((total / distData.value.total * 100).toFixed(1)) : 0
}
